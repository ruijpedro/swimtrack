...
class MainActivity : Activity() {
    ...
    private fun showAtleta() {
        ...
        content.addView(info("Ano", get("ano").ifBlank { "Por importar" }))
        content.addView(info("País", get("pais").ifBlank { "Por importar" }))
        content.addView(info("Clube", get("clube").ifBlank { "Por importar" }))
        content.addView(info("Escalão estimado", escalao(get("ano"))))
    ...
    private fun showImportar() {
        ...
        content.addView(
            info(
                "PDF Swimrankings",
                "Importa PDF de recordes pessoais. Reconhece piscina longa 50m e piscina curta 25m."
            )
        )
        content.addView(
            info(
        ...
        content.addView(button("📄 IMPORTAR PDF") {
        ...
        content.addView(button("🧪 COLAR TEXTO PDF / SWIMRANKINGS") {
            showColarTexto()
        })
    }
    ...
    private fun showColarTexto() {
        ...
        val caixa = inputMulti("Cola aqui texto do PDF ou do Swimrankings")
        content.addView(caixa)

        content.addView(button("📥 IMPORTAR TEXTO") {
    ...
    private fun showTempos() {
        ...
        content.addView(
            info(
                "Última importação",
                get("ultima_importacao").ifBlank { "Ainda sem importação." }
            )
        )
    ...
    private fun showTac() {
        ...
        val tacFonte = get("tac_fonte")
        if (tacFonte.isNotBlank()) {
            content.addView(info("Regulamento TAC", tacFonte))
        }

        val tacTexto = get("tac_texto")
        if (tacTexto.isNotBlank()) {
            content.addView(info("Resumo do regulamento importado", tacTexto))
        }

        val tempos = get("tempos")
        if (tempos.isBlank()) {
            content.addView(info("Sem tempos", "Importa primeiro os tempos da atleta."))
            return
        }
        ...
        content.addView(info("TAC atingidos", "$atingidos de $definidos TAC definidos."))
        content.addView(info("Objetivo mais próximo", objetivoMaisProximo()))
    }
    ...
    private fun importarTextoGeral(texto: String): String {
        if (texto.isBlank()) return "Texto vazio."

        return when {
            texto.contains("Swimrankings", true) ||
                    texto.contains("Piscina longa", true) ||
                    texto.contains("Piscina curta", true) ||
                    texto.contains("Recordes pess", true) -> {
                val total = importarPdfSwimrankings(texto)
                "PDF Swimrankings importado: $total tempos."
            }

            texto.contains("TAC", true) ||
                    texto.contains("Tempos de Admissão", true) ||
                    texto.contains("Regulamento", true) ||
                    texto.contains("FPN", true) ||
                    texto.contains("ANDL", true) -> {
                importarPdfRegulamentoTac(texto)
        ...
        private fun importarPdfSwimrankings(texto: String): Int {
        ...
        for (i in linhas.indices) {
            ...
            if (
                linha.contains("Piscina longa", true) ||
                (linha.contains("(50m)", true) && linha.contains("Piscina", true))
            ) {
                piscinaAtual = "50m"
                continue
            }
            ...
            if (
                linha.contains("Piscina curta", true) ||
                (linha.contains("(25m)", true) && linha.contains("Piscina", true))
            ) {
                piscinaAtual = "25m"
                continue
            }

            val modalidade = estiloDaLinha(linha)

            if (modalidade != null) {
                val item = extrairLinhaTempo(linha, modalidade, piscinaAtual)
                if (item != null) {
                    resultados.add(item)
                }
                continue
            ...
            val itemBloco = extrairBlocoTabela(linhas, i, piscinaAtual)
            if (itemBloco != null) {
                resultados.add(itemBloco)
            }
        }

        val melhores = escolherMelhores(resultados)

        prefs.edit()
            .putString("tempos", melhores)
            .putString(
                "ultima_importacao",
    ...
    private fun importarPerfil(texto: String) {
        val linhas = texto.lines()
            .map { it.trim() }
            .filter { it.isNotBlank() }

        val linhaPerfil = linhas.firstOrNull {
            it.contains("PEDRO", true) ||
                    it.contains("POR - Portugal", true)
        } ?: ""

        val nome = if (linhaPerfil.contains("PEDRO", true)) {
            linhaPerfil.substringBefore("2010").trim()
                .ifBlank { "PEDRO, Constanca Rolim" }
        } else {
            get("nome").ifBlank { "PEDRO, Constanca Rolim" }
        }

        val ano = Regex("\\b(20\\d{2}|19\\d{2})\\b")
            .find(linhaPerfil)
            ?.value
        ...
        val pais = if (texto.contains("POR - Portugal", true)) {
            "Portugal"
        } else {
        ...
        val clube = when {
            texto.contains("Assoc Desp Cult Rec Bairro dos Anjos", true) ->
                "Assoc Desp Cult Rec Bairro dos Anjos"

            texto.contains("Bairro dos Anjos", true) ->
    ...
    private fun estiloDaLinha(linha: String): String? {
        return when {
            linha.startsWith("Livres ", true) -> "Livres"
            linha.startsWith("Costas ", true) -> "Costas"
            linha.startsWith("Bruços ", true) -> "Bruços"
            linha.startsWith("Brucos ", true) -> "Bruços"
            linha.startsWith("Mariposa ", true) -> "Mariposa"
            linha.startsWith("Estilos ", true) -> "Estilos"
            else -> null
        }
    }
    ...
    private fun extrairLinhaTempo(
        ...
        val piscina = piscinaAtual.ifBlank {
            when {
                linha.contains("25m") -> "25m"
                linha.contains("50m") -> "50m"
                else -> ""
            }
        }

        if (piscina.isBlank()) return null

        val dist = Regex("\\b(50|100|200|400|800|1500)m\\b")
            .find(linha)
            ?.value
            ?: return null
        ...
        val data = Regex("\\b\\d{1,2}\\s+[A-Za-zÀ-ÿ]{3}\\s+\\d{4}\\b")
            .find(linha)
            ?.value
            ?: "-"

        val cidade = extrairCidade(linha.substringAfter(data, ""))

        val prova = "${dist.removeSuffix("m")} $estilo"
    ...
    private fun extrairBlocoTabela(
        ...
        if (piscinaAtual.isBlank()) return null

        val linha = linhas[i]

        val distMatch = Regex("^(50|100|200|400|800|1500)m$")
            .find(linha)
            ?: return null

        var estilo = procurarEstiloAnterior(linhas, i) ?: return null
        ...
        val bloco = linhas
            .subList(i, minOf(i + 8, linhas.size))
            .joinToString(" ")
        ...
        val cidade = extrairCidade(bloco.substringAfter(data, ""))

        val prova = "${distMatch.groupValues[1]} $estilo"
    ...
    private fun procurarEstiloAnterior(
        ...
        for (j in idx downTo maxOf(0, idx - 20)) {
            ...
            when {
                l.startsWith("Livres", true) -> return "Livres"
                l.startsWith("Costas", true) -> return "Costas"
                l.startsWith("Bruços", true) -> return "Bruços"
                l.startsWith("Brucos", true) -> return "Bruços"
                l.startsWith("Mariposa", true) -> return "Mariposa"
                l.startsWith("Estilos", true) -> return "Estilos"
            }
        }
    ...
    private fun escolherMelhores(lista: List<String>): String {
        ...
        for (linha in lista) {
            ...
            if (p.size >= 3) {
                ...
                if (antigo == null || tempoSeg(p[2]) < tempoSeg(antigo.split("|")[2])) {
                    mapa[chave] = linha
                }
            }
        }
    ...
    private fun importarPdfRegulamentoTac(texto: String) {
        val fonte = when {
            texto.contains("ANDL", true) -> "ANDL"
            texto.contains("FPN", true) ||
                    texto.contains("Federação Portuguesa", true) -> "FPN"

            else -> "Regulamento"
        }

        val resumo = texto.lines()
            .map { it.trim() }
            .filter {
                it.contains("TAC", true) ||
                        it.contains("Tempo", true) ||
                        it.contains("Admissão", true) ||
                        it.contains("Juvenil", true) ||
                        it.contains("Infantil", true) ||
                        it.contains("Júnior", true)
            }
            .take(40)
            .joinToString("\n")

        prefs.edit()
            .putString("tac_fonte", "$fonte importado")
            .putString("tac_texto", resumo)
            .apply()
    }
    ...
    private fun resumoTopo(): String {
        val nome = get("nome").ifBlank { "SwimTrack" }
